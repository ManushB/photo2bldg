...
namespace massrec {
	...
	double obj_function::operator() (const column_vector& arg) const {
		...
		std::vector<float> params2(params.begin() + 8, params.end());
		glWidget->renderImage(grammar, &params2, rendered_image, false, false);
		...
		if (refinement) {
			if (refinement_method == 0) {	// BOBYQA
				...
				for (int iter = 0; iter < maxIters; ++iter) {
					...
					cv::Mat rendered_image;
					std::vector<float> cur_params2(cur_params.begin() + 8, cur_params.end());
					glWidget->renderImage(grammar, &cur_params2, rendered_image);
					...
					for (int iter2 = 0; iter2 < 10; ++iter2) {
						for (int k = 0; k < cur_params.size(); ++k) {
							...
							std::vector<float> next_params1_2(next_params1.begin() + 8, next_params1.end());
							...
							std::vector<float> next_params2_2(next_params2.begin() + 8, next_params2.end());
							if (glWidget->renderImage(grammar, &next_params2_2, rendered_image2, true, true)) {
			...
			cv::Mat rendered_image;
			std::vector<float> best_params2(best_params.begin() + 8, best_params.end());
			glWidget->renderImage(grammar, &best_params2, rendered_image, false, false);
