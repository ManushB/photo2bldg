...
UShapeTaper::UShapeTaper(const std::string& name, const std::string& grammar_type, const glm::mat4& pivot, const glm::mat4& modelMat, float width, float depth, float height, float slope, float front_width, float back_height, const glm::vec3& color) {
	this->_active = true;
	this->_axiom = false;
	this->_name = name;
	...
	if (height / tanf(_slope / 180.0f * M_PI) > std::min(front_width, back_height) * 0.5f) {
		this->_scope.z = std::min(front_width, back_height) * 0.5f * tanf(_slope / 180.0f * M_PI);
	}
}

boost::shared_ptr<Shape> UShapeTaper::clone(const std::string& name) const {
	boost::shared_ptr<Shape> copy = boost::shared_ptr<Shape>(new UShapeTaper(*this));
	copy->_name = name;
	return copy;
}

void UShapeTaper::comp(const std::map<std::string, std::string>& name_map, std::vector<boost::shared_ptr<Shape> >& shapes) {
	float offset = _scope.z / tanf(_slope / 180.0f * M_PI);
	
	// top face
	if (name_map.find("top") != name_map.end() && name_map.at("top") != "NIL") {
		if (std::min(_front_width, _back_height) * 0.5f > offset) {
			glm::mat4 mat = glm::translate(_modelMat, glm::vec3(offset, offset, _scope.z));
			shapes.push_back(boost::shared_ptr<Shape>(new UShape(name_map.at("top"), _grammar_type, _pivot, mat, _scope.x - offset * 2, _scope.y - offset * 2, _front_width - offset * 2, _back_height - offset * 2, _color)));
		}
			...
			mat = glm::translate(_modelMat, glm::vec3(_scope.x - _front_width + offset, offset, _scope.z));
			shapes.push_back(boost::shared_ptr<Shape>(new Rectangle(name_map.at("top"), _grammar_type, _pivot, mat, _front_width - offset * 2, _scope.y - offset * 2, _color)));
		}
		else if (_back_height * 0.5f > offset) {
			glm::mat4 mat = glm::translate(_modelMat, glm::vec3(offset, _scope.y - _back_height + offset, _scope.z));
			shapes.push_back(boost::shared_ptr<Shape>(new Rectangle(name_map.at("top"), _grammar_type, _pivot, mat, _scope.x - offset * 2, _back_height - offset * 2, _color)));
		}
	}
	...
	if (name_map.find("side") != name_map.end() && name_map.at("side") != "NIL") {
		...
		points.clear();
		points.push_back(glm::vec2(0, 0));
		points.push_back(glm::vec2(_scope.y - _back_height, 0));
		points.push_back(glm::vec2(_scope.y - _back_height + offset, offset / cosf(_slope / 180.0f * M_PI)));
		points.push_back(glm::vec2(offset, offset / cosf(_slope / 180.0f * M_PI)));
		shapes.push_back(boost::shared_ptr<Shape>(new Polygon(name_map.at("side"), _grammar_type, _pivot, mat, points, _color, _texture)));

		mat = glm::rotate(glm::translate(_modelMat, glm::vec3(_front_width, _scope.y - _back_height, 0)), _slope / 180.0f * M_PI, glm::vec3(1, 0, 0));
		points.clear();
		points.push_back(glm::vec2(0, 0));
		points.push_back(glm::vec2(_scope.x - _front_width * 2, 0));
		points.push_back(glm::vec2(_scope.x - _front_width * 2 + offset, offset / cosf(_slope / 180.0f * M_PI)));
		points.push_back(glm::vec2(-offset, offset / cosf(_slope / 180.0f * M_PI)));
		shapes.push_back(boost::shared_ptr<Shape>(new Polygon(name_map.at("side"), _grammar_type, _pivot, mat, points, _color, _texture)));

		mat = glm::rotate(glm::rotate(glm::translate(_modelMat, glm::vec3(_scope.x - _front_width, _scope.y - _back_height, 0)), -M_PI * 0.5f, glm::vec3(0, 0, 1)), _slope / 180.0f * M_PI, glm::vec3(1, 0, 0));
		points.clear();
		points.push_back(glm::vec2(0, 0));
		points.push_back(glm::vec2(_scope.y - _back_height, 0));
		points.push_back(glm::vec2(_scope.y - _back_height - offset, offset / cosf(_slope / 180.0f * M_PI)));
		...
		mat = glm::rotate(glm::translate(_modelMat, glm::vec3(_scope.x - _front_width, 0, 0)), _slope / 180.0f * M_PI, glm::vec3(1, 0, 0));
		points.clear();
		points.push_back(glm::vec2(0, 0));
		points.push_back(glm::vec2(_front_width, 0));
		points.push_back(glm::vec2(_front_width - offset, offset / cosf(_slope / 180.0f * M_PI)));
...
void UShapeTaper::size(float xSize, float ySize, float zSize, bool centered) {
	_prev_scope = _scope;
...
void UShapeTaper::generateGeometry(std::vector<boost::shared_ptr<glutils::Face> >& faces, float opacity) const {
	...
	{
		std::vector<Vertex> vertices;
		glm::mat4 mat = _pivot * glm::rotate(glm::translate(_modelMat, glm::vec3(_front_width, _scope.y - _back_height, 0)), _slope / 180.0f * M_PI, glm::vec3(1, 0, 0));
		std::vector<glm::vec2> points;
		points.push_back(glm::vec2(0, 0));
		points.push_back(glm::vec2(_scope.x - _front_width * 2, 0));
		points.push_back(glm::vec2(_scope.x - _front_width * 2 + offset, offset / cosf(_slope / 180.0f * M_PI)));
	...
	{
		std::vector<Vertex> vertices;
		glm::mat4 mat = _pivot * glm::rotate(glm::rotate(glm::translate(_modelMat, glm::vec3(_scope.x - _front_width, _scope.y - _back_height, 0)), -M_PI * 0.5f, glm::vec3(0, 0, 1)), _slope / 180.0f * M_PI, glm::vec3(1, 0, 0));
		std::vector<glm::vec2> points;
		points.push_back(glm::vec2(0, 0));
		points.push_back(glm::vec2(_scope.y - _back_height, 0));
		points.push_back(glm::vec2(_scope.y - _back_height - offset, offset / cosf(_slope / 180.0f * M_PI)));
	...
	{
		std::vector<Vertex> vertices;
		glm::mat4 mat = _pivot * glm::rotate(glm::translate(_modelMat, glm::vec3(_scope.x - _front_width, 0, 0)), _slope / 180.0f * M_PI, glm::vec3(1, 0, 0));
		std::vector<glm::vec2> points;
		points.push_back(glm::vec2(0, 0));
		points.push_back(glm::vec2(_front_width, 0));
		points.push_back(glm::vec2(_front_width - offset, offset / cosf(_slope / 180.0f * M_PI)));
