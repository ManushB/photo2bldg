...
void FacadeG::decodeParams(float width, float height, int num_floors, int num_columns, std::vector<float> params, const std::vector<int>& selected_win_types, int mass_grammar_id, std::vector<float>& decoded_params) {
	...
	params[7] = std::max(0.02f, params[7]);	// TW
...
std::vector<cv::Scalar> FacadeG::getFacadeColors(const std::vector<float>& params, const cv::Mat& facade_img, float width, float height, int cluster_count) {
	...
	int SW = params[4] * 100 / width * facade_img.cols;
	cv::Mat copy_facade_img = facade_img.clone();
	cv::rectangle(copy_facade_img, cv::Rect(SW, 0, copy_facade_img.cols - SW * 2, copy_facade_img.rows), cv::Scalar(0, 0, 0), -1);
	ans.push_back(fs::getDominantColor(cv::Mat(copy_facade_img, cv::Rect(0, 0, copy_facade_img.cols, AH)), cluster_count));
	cv::imwrite("facade0.png", cv::Mat(copy_facade_img, cv::Rect(0, 0, copy_facade_img.cols, AH)));
	ans.push_back(fs::getDominantColor(cv::Mat(facade_img, cv::Rect(SW, 0, facade_img.cols - SW * 2, AH)), cluster_count));
	cv::imwrite("facade1.png", cv::Mat(facade_img, cv::Rect(SW, 0, facade_img.cols - SW * 2, AH)));
	ans.push_back(fs::getDominantColor(cv::Mat(copy_facade_img, cv::Rect(0, AH, copy_facade_img.cols, copy_facade_img.rows - AH - FH2 - GH)), cluster_count));
	cv::imwrite("facade2.png", cv::Mat(copy_facade_img, cv::Rect(0, AH, copy_facade_img.cols, copy_facade_img.rows - AH - FH2 - GH)));
	ans.push_back(fs::getDominantColor(cv::Mat(facade_img, cv::Rect(SW, AH, facade_img.cols - SW * 2, facade_img.rows - AH - FH2 - GH)), cluster_count));
	cv::imwrite("facade3.png", cv::Mat(facade_img, cv::Rect(SW, AH, facade_img.cols - SW * 2, facade_img.rows - AH - FH2 - GH)));
	ans.push_back(fs::getDominantColor(cv::Mat(copy_facade_img, cv::Rect(0, copy_facade_img.rows - FH2 - GH, copy_facade_img.cols, FH2)), cluster_count));
	cv::imwrite("facade4.png", cv::Mat(copy_facade_img, cv::Rect(0, copy_facade_img.rows - FH2 - GH, copy_facade_img.cols, FH2)));
	ans.push_back(fs::getDominantColor(cv::Mat(facade_img, cv::Rect(SW, facade_img.rows - FH2 - GH, facade_img.cols - SW * 2, FH2)), cluster_count));
	cv::imwrite("facade5.png", cv::Mat(facade_img, cv::Rect(SW, facade_img.rows - FH2 - GH, facade_img.cols - SW * 2, FH2)));
	...
	cv::imwrite("facade6.png", cv::Mat(copy_facade_img, cv::Rect(0, copy_facade_img.rows - GH, copy_facade_img.cols, GH)));
	ans.push_back(fs::getDominantColor(cv::Mat(facade_img, cv::Rect(SW, facade_img.rows - GH, facade_img.cols - SW * 2, GH)), cluster_count));
	cv::imwrite("facade7.png", cv::Mat(facade_img, cv::Rect(SW, facade_img.rows - GH, facade_img.cols - SW * 2, GH)));
