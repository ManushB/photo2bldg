...
public:
	static cv::Mat generateFacade(int width, int height, int thickness, int max_NF, int max_NC, std::vector<int>& selected_win_types, const std::vector<float>& params);
	static void decodeParams(float width, float height, int max_NF, int max_NC, const std::vector<float>& params, std::vector<int>& selected_win_types, std::vector<float>& decoded_params);
