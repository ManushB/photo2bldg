...
std::vector<cv::Scalar> FacadeE::getFacadeColors(const std::vector<float>& params, const cv::Mat& facade_img, float width, float height, int cluster_count) {
	std::vector<cv::Scalar> ans;

	int SW = params[3] * 100 / width * facade_img.cols;
	cv::Mat copy_facade_img = facade_img.clone();
	ans.push_back(fs::getDominantColor(cv::Mat(copy_facade_img, cv::Rect(0, 0, copy_facade_img.cols, copy_facade_img.rows)), cluster_count));
	ans.push_back(fs::getDominantColor(cv::Mat(facade_img, cv::Rect(SW, 0, facade_img.cols - SW * 2, facade_img.rows)), cluster_count));
