...
cv::Mat generateFacadeC(int width, int height, int thickness, std::pair<int, int> range_NF, std::pair<int, int> range_NC, int max_NF, int max_NC, const std::vector<float>& params) {
	...
	decodeParamsC(width, height, range_NF, range_NC, max_NF, max_NC, params, decoded_params);

	return generateFacadeC(1, decoded_params[0], decoded_params[1], width, height, thickness, decoded_params[2], decoded_params[3], decoded_params[4], decoded_params[5], decoded_params[6], decoded_params[7], decoded_params[8], decoded_params[9], decoded_params[10], decoded_params[11], decoded_params[12], decoded_params[13], decoded_params[14], decoded_params[15], decoded_params[16], decoded_params[17], decoded_params[18], decoded_params[19], decoded_params[20], decoded_params[21]);
}

void decodeParamsC(float width, float height, std::pair<int, int> range_NF, std::pair<int, int> range_NC, int max_NF, int max_NC, const std::vector<float>& params, std::vector<float>& decoded_params) {
	...
	decoded_params[12] = ND;
	decoded_params[13] = DT;
	decoded_params[14] = DH;
	decoded_params[15] = DW;
	decoded_params[16] = DS;
	decoded_params[17] = TWS;
	decoded_params[18] = TWW;
	decoded_params[19] = TWT;
	decoded_params[20] = TWH;
	decoded_params[21] = TWB;
}
...
cv::Mat generateRandomFacadeC(int width, int height, int thickness, std::pair<int, int> range_NF, std::pair<int, int> range_NC, std::vector<float>& params, int window_displacement, float window_prob) {
	...
	return generateFacadeC(1, NF, NC, width, height, thickness, WW, WH, WS, WT, WB, BS, TW, AH, FH, GH, ND, DT, DH, DW, DS, TWS, TWW, TWT, TWH, TWB, window_displacement, window_prob);
}
